...
if cmd[1]=="-c":
    ...
    ret=requests.post(url+"paste.php?access=api&id="+inputid)
    while ret.text[0]=="1":
        print("请输入密码")
        password=input()
        ret = requests.post(url + "paste.php?access=api&id=" + inputid,data={"password":password})
    if(ret.text[0]=="3"):
